...
describe('AdminUsersService', () => {
  ...
  beforeEach(() => {
    ...
    mockAdminUsersRepo = {
      listInstructorUsers: jest.fn(),
      getInstructorUserStats: jest.fn(),
    };
  ...
  it('마지막 접속일 기준으로 활성/비활성 상태를 계산해야 한다', async () => {
    (mockAdminUsersRepo.listInstructorUsers as jest.Mock).mockResolvedValue({
      totalCount: 3,
      users: [
        {
        ...
        {
          instructorId: 'instructor-boundary',
        ...
        {
          instructorId: 'instructor-inactive',
    ...
    const result = await service.listInstructorUsers({
      activityStatus: 'all',
      sessionStatus: 'all',
      page: 1,
      limit: 20,
